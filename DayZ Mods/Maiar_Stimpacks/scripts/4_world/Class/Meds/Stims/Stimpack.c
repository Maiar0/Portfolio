class Mar_Stimpack: Inventory_Base
{
	override void SetActions()
	{
		...
		AddAction(ActionGiveStimpackSelf);
	}
};

class StimpackMdfr: ModifierBase
{
	float m_RegenTime;
	override void Init()
	{
		...
		m_IsPersistent = true;
		m_ID = StimModifiers.MDF_STIMPACK
	...
	override void OnActivate(PlayerBase player)
	{
		...
		if(player.GetHealth("GlobalHealth", "Health") > 75)
